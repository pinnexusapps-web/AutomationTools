...
Public Sub Build_CC_Dashboard()
    ...
    With wsD
        ...
        .Range("A3").Value = "Click a dropdown value to ADD it; click an already-picked value again to REMOVE it. Click " & ALL_TAG & " to reset that row."
        .Range("A4").Value = "Locations pad to XXX-YY-ZZZ automatically (F01-8-5 finds F01-08-005). Category / Sub Category narrow once Parent Category is picked, like Excel's own filters."
    ...
    AddButton wsD, 416, 20, 100, 30, "Q-VEG", "QuickFilter_QVEG"
...
Public Sub ApplyFilters()
    ...
    If hit > 0 Then
        ...
        If hit > 1 Then
            wsD.Range(wsD.Cells(HDR_ROW + 1, R_COL), wsD.Cells(HDR_ROW + hit, R_COL + 2)).Sort _
                Key1:=wsD.Cells(HDR_ROW + 1, R_COL + 2), Order1:=xlAscending, _
                Header:=xlNo, MatchCase:=False, Orientation:=xlTopToBottom
        End If

        Dim stockCol As Long, lr As Long, tblAddr As String
        stockCol = FindStockCol(dat)
        If stockCol > 0 Then
            lr = UBound(dat, 1)
            tblAddr = "'" & wsP.Name & "'!" & _
                      wsP.Range(wsP.Cells(1, 1), wsP.Cells(lr, stockCol)).Address(True, True)

            Dim fx() As Variant, k As Long
            ReDim fx(1 To hit, 1 To 1)
            For k = 1 To hit
                fx(k, 1) = "=IFERROR(VLOOKUP(" & _
                           wsD.Cells(HDR_ROW + k, R_COL).Address(False, False) & _
                           "," & tblAddr & "," & stockCol & ",FALSE),"""")"
            Next k
            wsD.Cells(HDR_ROW + 1, R_COL + 3).Resize(hit, 1).Formula = fx
        Else
            wsD.Cells(HDR_ROW, R_COL + 3).Value = "STOCK (col n/a)"
        End If
