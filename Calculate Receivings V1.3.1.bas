Attribute VB_Name = "CalculateReceivings_V1_3"
...
FindSKULabel:
    ...
    ' ---- Locate the columns we need by their header text ----
    ...
    ' ---- Step 1: Rebuild "Received Quantity" with the corrected VLOOKUP ----
    ...
    ' ---- Step 2: Delete the columns that are no longer needed ----
    ...
    ' ---- Step 3: Sort A-Z by Supplier SKU (whole rows move together) ----
    ...
    ' ---- Step 4: Add the new columns (Invoice Qty, TCN, Gross Cost, Barcode) ----
    ' These are placed right after "Unit Cost". If they already exist from a
    ' previous run, the macro reuses them instead of adding duplicates.
    Dim colInvoiceQty As Long, colTCN As Long, colGrossCost As Long, colBarcode As Long

    colInvoiceQty = FindHeaderCol(wsMain, skuRow, skuCol, lastCol + 4, "Invoice Received Qty")
    ...
    Dim rqColLetter As String, ucColLetter As String, invColLetter As String
    ...
    For r = skuRow + 1 To lastRow
        ' TCN = Invoice Received Qty - Received Quantity
        wsMain.Cells(r, colTCN).Formula = "=" & invColLetter & r & "-" & rqColLetter & r

        ' Gross Cost = Received Qty * 1.05 * Unit Cost
        wsMain.Cells(r, colGrossCost).Formula = _
            "=" & rqColLetter & r & "*1.05*" & ucColLetter & r

        ' Barcode via VLOOKUP against the raw data sheet
        wsMain.Cells(r, colBarcode).Formula = _
            "=VLOOKUP(" & skuColLetter & r & ", '" & wsData.Name & "'!$L:$AB, 5, 0)"
    Next r

    ' ---- Highlight rows where TCN > 0 ----
    ...
    highlightRange.FormatConditions.Add Type:=xlExpression, _
        Formula1:="=$" & tcnColLetter & (skuRow + 1) & ">0"
    ...
    ' ---- Total Gross Cost (replaces the old Total Received Quantity total) ----
    Dim gcColLetter As String
