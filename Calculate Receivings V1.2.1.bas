Attribute VB_Name = "CalculateReceivings_V1_2_1"
Sub CalculateReceivings()
    ...
    Dim skuColLetter As String, pColLetter As String, qColLetter As String
    Dim prodNameColLetter As String
...
FindSKULabel:
    ...
    wsMain.Range(wsMain.Cells(1, skuCol + 1), wsMain.Cells(1, skuCol + 13)).ColumnWidth = 2.5
    
    prodNameColLetter = Split(wsMain.Cells(1, skuCol + 1).Address, "$")(1)
    wsMain.Columns(prodNameColLetter).AutoFit
    ...
    wsMain.Range(qColLetter & (skuRow + 1) & ":" & qColLetter & lastRow).Formula = _
        "=1.05 * " & pColLetter & (skuRow + 1) & " * VLOOKUP(" & skuColLetter & (skuRow + 1) & ", '" & wsData.Name & "'!$L:$AB, 17, 0)"
    
    With wsMain.Range(qColLetter & (skuRow + 1) & ":" & qColLetter & lastRow)
        .Value = .Value
    End With
    ...
    wsMain.Columns(qColLetter).AutoFit
